...
variable "enable_rtce" {
  description = <<-EOT
    ...
    you toggle it in the Confluent Cloud Console yourself (both car_telemetry and
    car_state). Set TF_VAR_enable_rtce=false for an org or region where RTCE isn't
    available, to skip the key.
  EOT
  type        = bool
  default     = true
}
