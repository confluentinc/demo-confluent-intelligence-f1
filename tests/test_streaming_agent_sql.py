...
def test_car_state_uses_one_60_second_window_per_race_lap():
    for relative_path in (
        ...
        assert "TUMBLE(TABLE enriched, DESCRIPTOR(event_time), INTERVAL '60' SECOND)" in sql
...
def test_forecast_uses_the_same_one_per_lap_window():
    ...
    window = "TUMBLE(TABLE `car_telemetry`, DESCRIPTOR(event_time), INTERVAL '60' SECOND)"
