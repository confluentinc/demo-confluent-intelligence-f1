...
if [ -f "$SCRIPT_DIR/.race-task-arn" ]; then
  ...
  aws ecs stop-task --region us-east-2 --cluster "$CLUSTER" --task "$TASK_ARN" --reason "Manual stop"
  rm "$SCRIPT_DIR/.race-task-arn"
  echo "Race stopped."
else
  echo "No running race found. Checking ECS..."
  aws ecs list-tasks --region us-east-2 --cluster "$CLUSTER" --desired-status RUNNING
fi
