...
def _check_node_version() -> None:
    """Abort if Node is missing or too old; warn if no prebuilt binary exists for this version."""
    try:
    ...
    version_str = ver_result.stdout.strip().lstrip("v")  # e.g. "25.9.0"
    major = int(version_str.split(".")[0])
    abi = int(abi_result.stdout.strip())               # e.g. 141
...
def _ensure_local_mcp(project_root: Path) -> Path:
    """
...
def main():
    ...
    dist_js = _ensure_local_mcp(project_root)
