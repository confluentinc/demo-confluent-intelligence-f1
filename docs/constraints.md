...
- Race simulator runs as an ECS Fargate service per attendee with `RACE_LOOP=true` so the feed is always live; instructors control fleets via `uv run workshop start-races` / `stop-races` (the `start-all-races` / `stop-all-races` scripts remain as deprecated aliases), and a single deployment via `uv run race`
