...
FIA Timing Feed (external) ──→ IBM MQ ──→ MQ Connector ──→ race-standings-raw
                                                        ...
                                                        - Set car_number as key
                                                            ...
                                                            race-standings
                                                                   │
Race History (9 GPs)       ──→ Postgres ──→ CDC Debezium ──→ driver_race_history ──→ Tableflow ──→ Lakehouse
                                                                   │
                                          ┌────────────────────────┘
                                          ▼
...
8. **Tableflow on two topics** — `pit-decisions` and `driver_race_history` materialize to Delta Lake for Genie analytics.
...
198 rows (22 drivers × 9 prior races) of fictional season-to-date race history. Postgres on EC2 (Docker), pre-loaded during Terraform provisioning from `data/driver_race_history_seed.sql` (auto-generated by `data/generate_driver_race_history.py`, `random.seed(42)` for determinism). CDC Debezium Connector streams to Kafka. Composite PK `(race_id, car_number)`. Schema: race_id, gp_name, race_date, car_number, driver, team, starting_grid, finishing_pos, positions_gained, pit_stops, stint_1_tire, stint_2_tire, stint_3_tire.
...
| `deployment_id` | Alphanumeric, max 8 chars (e.g. `PROD` or initials). Drives the `RIVER-RACING-${deployment_id}` naming prefix. Used verbatim in CC display names (uppercase) and lowercased for AWS resources |
| `aws_bedrock_access_key` | AWS Bedrock Access Key for Flink AI agent connections |
| `aws_bedrock_secret_key` | AWS Bedrock Secret Key for Flink AI agent connections |
| `aws_session_token` | Optional. Required only if Bedrock key starts with `ASIA*` (temporary credentials) |

Region is hardcoded to `us-east-1` (RTCE availability). All resources use the branded prefix `RIVER-RACING-${deployment_id}` — uppercase for CC display names, lowercased via `lower()` inside AWS modules (S3/ECR require lowercase). AWS credentials use the default provider chain (`~/.aws/credentials`, env vars, or IAM role).
...
**Question 2 — Tire Strategy Correlation:** *"What's James River's average position gain per tire sequence?"* Uses `driver_race_history` table. Expected answer: `SOFT-MEDIUM` averages **+2.75** positions over 4 races; other strategies average **−2.4** over 5 races. Validates the agent's lap-33 MEDIUM pit recommendation.
...
- Deploy CDC Debezium Connector → creates `driver_race_history` topic
...
- Enable Tableflow on `pit-decisions` and `driver_race_history`
...
├── data/                               # driver_race_history_seed.sql + generate_driver_race_history.py (Postgres seed data)
...
- Tableflow on pit-decisions + driver_race_history
...
20. **Branded naming convention** — Single `name_prefix = "RIVER-RACING-${var.deployment_id}"` local in both `terraform/core/main.tf` and `terraform/demo/main.tf` drives every resource name. CC resources use the prefix verbatim with uppercase suffixes (`-ENV`, `-CLUSTER`, `-app-key`, etc.). AWS modules accept `var.name_prefix` and apply `lower()` internally for S3/ECR/IAM/ECS. With `deployment_id=PROD`: CC env = `RIVER-RACING-PROD-ENV`, S3 bucket = `river-racing-prod-tableflow-<hex>`. Tableflow S3 bucket appends a `random_id` hex suffix for global uniqueness. AWS resources tagged with `owner_email`.
38. **Don't rebuild resource names from locals in outputs** — `core/outputs.tf` originally had `cluster_name = "${local.name_prefix}-cluster"`, which silently drifted from the actual resource display name when the suffix changed (`-cluster` → `-CLUSTER`). The Flink statement that creates `car-telemetry` then failed with "current database not set" because `sql.current-database` referenced the stale lowercase name. Fix: outputs should read from `module.cluster.cluster_name` which sources from `confluent_kafka_cluster.main.display_name` — single source of truth.
39. **AWS modules take `name_prefix`, not `deployment_id`** — Earlier convention had each AWS module (`mq`, `postgres`, `ecs`, `tableflow`) take `var.deployment_id` and rebuild `f1-demo-${deployment_id}-<role>` internally. Refactored so modules accept `var.name_prefix` directly (built once in `terraform/demo/main.tf` locals) and use `${lower(var.name_prefix)}-<role>`. Eliminates duplicate prefix construction and means the brand can change in one place.
40. **Tableflow `bucket_name` variable removed** — Now derived inside the module as `${lower(var.name_prefix)}-tableflow-${random_id.suffix.hex}` instead of being passed in. Same for `role_name` and `policy_name`. One less variable to wire through.
41. **`deployment_id` allows uppercase** — Validation in `deploy.py` was originally lowercase-only. Relaxed to accept any alphanumeric ≤8 chars so users can type `PROD` directly for demo recordings. Lowercase only happens inside AWS modules where the AWS API requires it.
42. **Postgres table is `driver_race_history`, not `race_results`** — Originally `race_results`; renamed to make grain explicit (one row per driver per race, prior-season data). The CDC Reroute SMT propagates the table name to the Kafka topic and downstream Tableflow Delta Lake table, so Genie queries reference `f1_demo.driver_race_history`.
